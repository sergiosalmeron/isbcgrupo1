...
public class NuestroExtractor
{
    ...
    {
	org.apache.commons.logging.LogFactory.getLog(NuestroExtractor.class).info("Extracting cases information.");
	ProgressController.init(NuestroExtractor.class, "Extracting cases information ...", cases.size());
	for (CBRCase c : cases)
	{
	    ...
	    extractInformation(c.getDescription());
	    extractInformation(c.getDescription());
	    ProgressController.step(NuestroExtractor.class);
	}
	ProgressController.finish(NuestroExtractor.class);
    }
    ...
    {
	org.apache.commons.logging.LogFactory.getLog(NuestroExtractor.class).info("Extracting query information.");
	...
	try
	{
		...
		if (at.getType().equals(String.class))
		{
			...
			org.apache.commons.logging.LogFactory.getLog(NuestroExtractor.class).debug("Adding features to attribute: "+at.getName()+" <- "+value);
		    }
		...
		} else if (at.getType().equals(Boolean.class))
		{
			...
			if (p.getPhrase().equalsIgnoreCase(name))
			{
			    ...
			    org.apache.commons.logging.LogFactory.getLog(NuestroExtractor.class).debug("Enabling attribute: "+at.getName()+". Source: "+p.getPhrase());
	...
	{
	    org.apache.commons.logging.LogFactory.getLog(NuestroExtractor.class).error(e);
