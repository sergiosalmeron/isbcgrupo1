 ...
 * 
 */
public class OpennlpPOStaggerSpanish
{
    ...
    {
	org.apache.commons.logging.LogFactory.getLog(OpennlpPOStaggerSpanish.class).info("OpenNLP POS tagging.");
	ProgressController.init(OpennlpPOStaggerSpanish.class, "OpenNLP POS tagging", cases.size());
	for(CBRCase c: cases)
	{
	    ...
	    ProgressController.step(OpennlpPOStaggerSpanish.class);
	}
	ProgressController.finish(OpennlpPOStaggerSpanish.class);
    }
    ...
    public static void tag(IETextOpenNLP text)
    {
		...
		for(Sentence sentence: text.getAllSentences())
		{
			List<Token> tokens = sentence.getTokens();
			ArrayList<String> stringTokens = new ArrayList<String>();
			List tags = null;
			for(Token t: tokens)
				stringTokens.add(t.getRawContent());
			tags = tagger.tag(stringTokens);
			
			for(int i=0; i<stringTokens.size(); i++)
			{
				Token t = tokens.get(i);
				t.setPostag((String)tags.get(i));
			}
		}
    }
    ...
    private static POSTaggerME spanishPOStagger = null;
